...
// var API_SERVER = 'https://t2.uma.com/';
var API_SERVER = 'https://mofisher-wapp.uma.com/';
...
function httpPost(req) {
	...
	wx.getStorage({
		...
		success: function (res) {
			var token = res.data;
			if (token != null) {
				req.param['open_id'] = '' + token;//在这里赋值token
				...
				wx.request({
					...
					success: function (result) {
						if (result.statusCode == 200) {//接口调用返回伟200的时候才代表成功
							...
							wx.showModal({
								title: '错误提示',
								content: 'request warning [ ' + result.statusCode + ' : ' + result.errMsg + ' ] ',
								showCancel: false,
								confirmText: '我知道了',
							})
